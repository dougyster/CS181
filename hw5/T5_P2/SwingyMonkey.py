...
class SwingyMonkey:
    ...
    def get_state(self):
        ...
        return {
            "score": self.score,
            "tree": {
                "dist": next_tree["x"] + 215 - self.monkey_right,
            ...
            "monkey": {
                ...
                - self.monkey_loc
                + self.monkey_img.get_height() / 2,
                "bot": self.screen_height
                - self.monkey_loc
                - self.monkey_img.get_height() / 2,
            },
        }
    ...
    def game_loop(self):
        ...
        if self.iter < self.background_img.get_width() - self.screen_width:
            self.screen.blit(
                self.background_img, (self.iter + self.background_img.get_width(), 0)
            )
            ...
            self.trees.append(
                {
                    ...
                    "y": int(
                        (0.3 + npr.rand() * 0.65) * (self.screen_height - self.tree_gap)
                    ),
                    "s": False,
                }
            )
        ...
        for tree in self.trees:
            ...
            # Compute collision.
            if (
                (trunk_left < (self.monkey_left + 15))
                and (trunk_right > (self.monkey_left + 15))
            ) or (
        ...
        if self.text is not None:
            ...
            self.screen.blit(
                text, (self.screen_width - textpos[2], 0, textpos[2], textpos[3])
            )
